...
const HeaderCart = (props: Props) => {
  ...
  return (
    <div className={cn('relative', isShaking ? 'cart-shake' : '')}>
      <Link href={`/user/cart`} className="cursor-pointer">
        <div>
          ...
          {count > 0 && (
            <span className="absolute -end-2.5 -top-2.5 flex h-5 min-w-5 items-center justify-center rounded-full border border-white bg-red-600 px-1 text-xs font-medium text-white shadow">
              {count}
            </span>
          )}
        </div>
